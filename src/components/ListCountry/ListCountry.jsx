...
function ListCountry({ countryData, onSearchUpdate,etatSearch }) {
  ...
  // Fonction pour envoyer la valeur de recherche au parent
  const handleSubmit = () => {
    onSearchUpdate(searchValue);
  };


   // Effet pour réinitialiser le champ de recherche lorsque etatSearch est false
   useEffect(() => {
    if (!etatSearch) {
      onSearchUpdate(searchValue); 
      setSearchValue(""); // Nettoie l'input

    }
  }, [etatSearch, onSearchUpdate]);
